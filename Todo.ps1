...
If($Choice.GetEnumerator().Name.contains($String)){
 ...
 Return [ScriptBlock]{Write-Host "Choose a number from the menu"; ShowMenu}
    }
}
...
Else
{Write-Host ('Could not find taskname {0} ' -f ($OldTaskName))}
...
Else
{Write-Host 'Could not find task '$TaskName}
